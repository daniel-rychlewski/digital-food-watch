...
- **MainActivity**: This activity is displayed after loading. It shows all chips: CartChip, AddressChip, ModeChip, FoodOverviewChip, FoodChip, TermsOfUseChip, ImprintChip.
...
- **AddressChip, CartChip, FoodChip, FoodOverviewChip, ImprintChip, ModeChip, OptionChip, OptionMultiChip, TermsOfUseChip**: Clickable chips, leading to their respective activity.
- **ChipUtils**: Chip utility functions: getAvailabilityRestrictions, getDaySpecificText, getMinimumOrderValuePrice, getMinimumOrderValueText, isRestaurantCurrentlyOpen, isUnavailable, isWrongDay, satisfiesTimeCriteria.
- **InactiveScreen, InternalError, RestaurantClosedError, UpdateError, LoginError, InternetError**: screens that are displayed when the respective error has occurred.
...
- **FirestoreAPI**: Contains functions to fetch the Firestore document with.
...
- **AppIcon**: The app icon used for the store listing
- **SplashScreen**: The app icon used when starting the app. You can generate it using https://www.appicon.co/#image-sets .
- **Food**: The folder for images referenced in the background field of a food or options item.

### Miscellaneous
- **Localizable.xcstrings**: Messages prior to loading from Firebase are internationalized here.
- **PrivacyInfo.xcprivacy**: Privacy-related access declarations

## Creating the app
- Replace the GoogleService-Info.plist with yours so that the app can access the Firebase database and storage. Get yours in Firebase's project settings -> Add app -> iOS apps -> Download GoogleService-Info.plist.
- Constants: insert collection, document, global collection, global document, country, local language. Make sure your Google cloud user can access the database with the rules that you set online.
- FirestoreAPI: adjust the apiKey (from GoogleService-Info.plist), baseUrl, email and password.
